<!DOCTYPE html>
<html lang="en">
    <head>
        <title>A. Mitchell Bullard - Screencast</title>
        <?php include '_/components/php/common_content/head_tag_contents.php'; ?>
    </head>
    <body id="body_screencast">
        <section  class="main_container  container">
        <!-- header -->
        <?php include '_/components/php/common_content/header.php'; ?>
        <!-- navigation -->
        <?php include '_/components/php/common_content/navigation.php'; ?>

        <!-- content -->
        <section class="content_container  container  col  col-xxs-12">
            <div class="row">
                <section class="content_container  container  col  col-xxs-12 col-md-10 col-md-offset-1 col-lg-8 col-lg-offset-2">
                    <h5>The first in a series of programming screencasts for my daughter is to be released early January.</h5>
                    <br>
                    <p>
                        If you're new to programming, or you're looking for a great spot for you're kid to
                        learn the awesomeness of programming, keep an eye on this page.
                    </p>
                </section>
            </div> <!-- row -->
        </section> <!-- content_container -->

        <!-- footer -->
        <footer class="col col-sm-12">
            <div class="row">
                <hr>
                <?php include '_/components/php/common_content/footer_content.php'; ?>
            </div> <!-- row -->
        </footer>

        </section> <!-- main_container -->
    <?php include '_/components/php/common_content/script_tags.php'; ?>
    </body>
</html>