<!DOCTYPE html>
<html lang="en">
  <head>
    <title>A. Mitchell Bullard - Contact</title>
    <?php include '_/components/php/common_content/head_tag_contents.php'; ?>
  </head>
  <body id="body_contact">
    <section  class="main_container  container">


<!-- header --> 
      <?php include '_/components/php/common_content/header.php'; ?>


<!-- navigation -->
      <?php include '_/components/php/common_content/navigation.php'; ?> 


<!-- content -->
      <section class="content_container  container  col  col-xxs-12">
        
        <div class="col  col-xxs-12">
          <div class="row">
                    
             <a href="https://twitter.com/TopShelfBullard" target="_blank">
              <div id="twitter" class="twitter  col  col-xxs-6   col-xs-3  col-sm-2  col-sm-offset-0">         
                <div class="bw_icon">
                  <img  src="images/icons/twitter.png" alt="black and white twitter icon" width="100%">
                </div>
                <div class="color_icon">
                  <img  src="images/icons/twitter-icon.png" alt="twitter icon" width="100%"> 
                </div>                 
              </div>
            </a>           

            <a href="https://github.com/TopShelfBullard" target="_blank">
              <div id="github" class="github  col  col-xxs-6  col-xs-3  col-xs-offset-1   col-sm-2  col-sm-offset-2">             
                <div class="bw_icon">
                  <img src="images/icons/github.png" alt="black and white github icon" width="100%">
                </div>
                <div class="color_icon">
                  <img  src="images/icons/github-icon.png" alt="github icon"  width="100%">
                </div>             
              </div>
            </a>


            <a href="mailto:amb@amitchellbullard.com" target="_blank">
              <div id="mail" class="mail  col  col-xxs-6  col-xxs-offset-3  col-xs-3  col-xs-offset-1   col-sm-2  col-sm-offset-2">            
                <div class="bw_icon">
                  <img  src="images/icons/mail.png" alt="black and white mail icon" width="100%">
                </div>
                <div class=" mail_color_icon">
                  <img  src="images/icons/mail-icon.png" alt="mail icon" width="100%">
                </div>           
              </div>
            </a>

          </div>
        </div>



        <div class="col  col-xxs-12">
          <div class="row">

            <a href="https://plus.google.com/u/0/116475027692126557852/posts" target="_blank">
              <div id="googleplus" class="googleplus  col  col-xxs-6  col-xs-3  col-xs-offset-1   col-sm-2  col-sm-offset-2">             
                <div class="bw_icon">
                  <img  src="images/icons/googleplus.png" alt="black and white google plus icon" width="100%">
                </div>
                <div class="color_icon">
                  <img  src="images/icons/googleplus-icon.png" alt="google plus icon" width="100%">
                </div>              
              </div>
            </a>


            <a href="https://www.facebook.com/a.mitchell.bullard" target="_blank">
              <div id="facebook" class="facebook  col  col-xxs-6  col-xs-3  col-xs-offset-1   col-sm-2  col-sm-offset-2">             
                <div class="bw_icon">
                  <img  src="images/icons/facebook.png" alt="black and white facebook icon" width="100%">
                </div>
                <div class="color_icon">
                  <img  src="images/icons/facebook-icon-alt.png" alt="facebook icon" width="100%">
                </div>          
              </div>
            </a>  


            <a href="http://www.linkedin.com/pub/a-mitchell-bullard/72/a42/909" target="_blank">
              <div id="linkedin" class="linkedin  col  col-xxs-6  col-xxs-offset-3  col-xs-3  col-xs-offset-1   col-sm-2   col-sm-offset-2">              
                <div class="bw_icon">
                  <img  src="images/icons/linkedin.png" alt="black and white linkedin icon" width="100%">
                </div>
                <div class="color_icon">
                  <img  src="images/icons/linkedin-icon.png" alt="linkedin icon" width="100%">
                </div>
              </div>
            </a>



          </div>
      </section> <!-- content_container -->

<!-- footer -->
      <footer>
        <hr>
        <div class="row">
          <?php include '_/components/php/common_content/footer_content.php'; ?>
        </div> <!-- row -->
      </footer>   
    

    </section> <!-- main_container -->
    <?php include '_/components/php/common_content/script_tags.php'; ?>
  </body>
</html>